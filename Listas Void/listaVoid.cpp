...
Nodo crearNodo(ListaVoid lista, void * dato){
    ...
    Nodo nodo = new NodoE;
    nodo->dato = malloc(sizeof(lista->tamDelDato)); /// Recordar que hay que asociar el tamanio del dato al de la lista

    if(!nodo->dato){
      delete(nodo);
      return nodo;
    }
    //Esta funcion lo que hace es castear en base al espacio que ocupa y no por el tipo de dato
    //ejemplo: mi dato ahora va a ocupar esto, no importa lo que sea
    memcpy(nodo->dato,dato,lista->tamDelDato); //destino,origen,tama�o de los punteros en bytes
...
void * obtenerDato(ListaVoid lista, int pos){
    ...
    /*if(!lista || lista == NULL || pos<0 || pos>=lista->tamanio || lista->tamanio<=0){
        return NULL;
    }*/
...
void mostrarLista(ListaVoid lista){
    ...
    for(int i=0 ; i<lista->tamanio ; i++){
        Persona p = *((Persona*)obtenerDato(lista,i)); //int info = *((int*) obtenerDato(lista,i));
        mostrarPersona(p);                             //printf (" %d ",info);
    }
}
...
int insertarInicio(ListaVoid lista, void * dato){
    ...
    if(lista->tamanio==0){
        lista->inicio = lista->ultimo = nuevoNodo;
        nuevoNodo->prox = NULL;
        lista->tamanio++;
        return 1;
    }

    nuevoNodo->prox = lista->inicio;
...
int insertar(ListaVoid lista, void * dato, int pos){
    ...
    if(pos == lista->tamanio){
        return insertarFinal(lista,dato);
    }
    ...
    /*if(n == NULL || !lista || lista==NULL || pos<0 || pos>lista->tamanio){
        return -1;
    }*/

    nuevoNodo->prox = aux->prox;    //Nodo n = crearNodo(dato,aux->prox);
    aux->prox = nuevoNodo;          //aux->prox = n;
    lista->tamanio++;
...
int remover(ListaVoid lista, int pos, void * backup){
    ...
    /*if(!lista || lista==NULL || pos<0 || pos > lista->tamanio){
        return -1;
    }*/
